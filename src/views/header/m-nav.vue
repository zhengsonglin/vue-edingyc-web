    ...
    <ul>
      <li class="nav-li" :class="[item.className, { line: item.hasLine }]" v-for="(item, index) in navList">
        <div @click="setNavPanel(item, index)" :class="{ active: item.isActive }">
          <p class="wrapNav text-overellipsis" :test="item.iconActive">
    ...
    <div class="content w750" :class="{ active: show }">
      ...
      <div key="brand" class="panel-nav brandGroup" :class="{ active: brandPanel.showSub }" v-show="brandPanel.show || brandPanel.showSub">
        <scroll ref="brandScroll" class="list-content" :data="brandList.sortData">
          <ul>
            <li class="first" v-show="brandList.sortData.length"><p class="active" @click="clearSelect(0)">全部品牌</p></li>
        ...
        <scroll ref="subBrand" class="list-content list-model" :data="subBrandList">
          <ul>
            <template v-for="(obj, index) in subBrandList">
              ...
              <li class="normal-item">
                <p class="single-item" v-for="(item, n) in obj.items" @click="checkedSubBrand(item)">
                  <span>{{ item.name }}</span>
                </p>
              </li>
            </template>
          ...
          <ul>
            <li class="first" v-show="modelList.length">
              <p @click="clearSelect(1)">
                <x-icon type="ios-checkmark-empty" class="icon-checked" size="42" v-show="!navList[1].isActive"></x-icon>
                全部车型
              </p>
            </li>
            <template v-for="(obj, index) in modelList">
              ...
              <li class="normal-item">
                <p class="single-item" v-for="(item, n) in obj.items" @click="checkedModel(item)">
                  <x-icon type="ios-checkmark-empty" class="icon-checked" size="42" v-show="initLastChecked(1, item.desc)"></x-icon>
                  <span>{{ item.brand }} {{ item.year }}款 {{ item.desc }}</span>
                </p>
              </li>
            </template>
          </ul>
            ...
            <ul>
              <li class="first">
                <p @click="clearSelect(2)">
                  <x-icon type="ios-checkmark-empty" class="icon-checked" size="42" v-show="!navList[2].isActive"></x-icon>
                  全部颜色
                </p>
              </li>
              <li class="normal-item">
...
export default {
  ...
  data() {
    return {
      ...
      insuranceList: [{ key: '0', value: '全部' }, { key: '1', value: '不看带保险' }, { key: '2', value: '只看带保险' }],
      specialPrice: { key: '0', value: '全部' }, //特价
      specialPriceList: [{ key: '0', value: '全部' }, { key: '1', value: '不看特价车' }, { key: '2', value: '只看特价车' }]
    };
  },
  computed: {
    brandIconAction() {
  ...
  methods: {
    ...
    getColorList() {
      this.$http.post('/resource/colorList').then(res => {
        let datas = res.data.dataList.slice(),
          result = [];
        datas.forEach((item, index, arrs) => {
          let excolor = arrs[index].excolor,
            len = excolor.length;
    ...
    _normalizeBrand(list, groupKey, sortType = 'asc') {
      //数据列表, 分组的key值, 升序或降序
      let map = {};
...
.slideDefine-enter-active,
.slideDefine-leave-active {
  transition: all 0.5s;
